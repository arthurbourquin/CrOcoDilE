...
function myScript(thisObj){
    function myScript_buildUI(thisObj){
        ...
        myPanel.grp.groupA.ButtonOne.onClick = function() {
            let S = 1;
            let F = 1;
            if(ScriptUI.environment.keyboardState.altKey == 1){S = -1}else{S = 1};
            if(ScriptUI.environment.keyboardState.shiftKey == 1){F = 10}else{F = 1};
...
function AddFrame(factor) {
    ...
    for(i=1;i<=NbSL;i++){
        ...
        app.project.activeItem.selectedLayers[i-1].startTime + (i-factor)*app.project.activeItem.frameDuration;
